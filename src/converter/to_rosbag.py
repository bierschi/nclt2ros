...
from src.transformer.sensor_data import SensorData
...
class ToRosbag:
    ...
    USAGE:
            ConvertToRosbag('2013-01-10', 'example.bag', cam_folder=None)
    ...
    def __init__(self, date, bag_name, cam_folder=None):
        ...
        self.dataset       = ReadRawData(self.date)
        self.sensor_data   = SensorData(self.date)
        self.laser_data    = HokuyoData(self.date)
        self.sync_velodyne = VelodyneSyncData(self.date)
    ...
    def process(self):
        ...
        # load sensor data
        gps_list      = self.dataset.read_gps_csv(all_in_one=True)
        gps_rtk_list  = self.dataset.read_gps_rtk_csv(all_in_one=True)
        ms25_list     = self.dataset.read_ms25_csv(all_in_one=True)
        odom_list     = self.dataset.read_odometry_mu_100hz_csv(all_in_one=True)
        odom_cov_list = self.dataset.read_odometry_cov_100hz_csv(all_in_one=True)
        wheels_list   = self.dataset.read_wheels_csv(all_in_one=True)
        kvh_list      = self.dataset.read_kvh_csv(all_in_one=True)
        gt_list       = self.dataset.read_gt_csv(all_in_one=True)
        gt_cov_list   = self.dataset.read_gt_cov_csv(all_in_one=True)

        # load hokuyo data
        utime_hok4, data_hok4   = self.laser_data.read_next_hokuyo_4m_packet()
        utime_hok30, data_hok30 = self.laser_data.read_next_hokuyo_30m_packet()

        # load velodyne sync data
        vel_sync_timestamps_microsec, vel_sync_bin_files = self.sync_velodyne.get_velodyne_sync_timestamps_and_files()
        ...
        while True:
            ...
            if next_packet == "done":
                ...
                print("write gps")
                navsat, track, speed, timestamp, tf_static_msg = self.sensor_data.gps_to_navsat(gps_list=gps_list, i=i_gps)
                ...
                print("write gps_rtk")
                navsat, track, speed, timestamp, tf_static_msg = self.sensor_data.gps_rtk_to_navsat(gps_rtk_list=gps_rtk_list, i=i_gps_rtk)
                ...
                print("write ms25")
                imu, mag, timestamp, tf_static_msg = self.sensor_data.ms25_to_imu(imu_list=ms25_list, i=i_ms25)
                ...
                print("write odom")
                odom, timestamp, tf_msg, tf_static_msg = self.sensor_data.wheel_odom_to_odometry(odom_list=odom_list, odom_cov_list=odom_cov_list, wheels_list=wheels_list, kvh_list=kvh_list, i=i_odom)
                ...
                print("write gt")
                gt, timestamp, tf_static_msg = self.sensor_data.gt_to_odometry(gt_list=gt_list, gt_cov_list=gt_cov_list, i=i_gt)
                ...
                print("write hok4")
                timestamp, scan, tf_static_msg = self.laser_data.write_hokuyo_4m_to_laserscan(utime_hok4, data_hok4)
                ...
                print("write hok30")
                timestamp, scan, tf_static_msg = self.laser_data.write_hokuyo_30m_to_laserscan(utime_hok30, data_hok30)
                ...
                print("vel_sync")
                try:
                    hits = self.sync_velodyne.read_next_velodyne_sync_packet(vel_sync_bin_files[i_vel])
                except ValueError:
                    print("Error velodyne")

                timestamp, pc2_msg, tf_static_msg = self.sync_velodyne.xyzil_array_to_pointcloud2(utime=vel_sync_timestamps_microsec[i_vel], hits=hits)
...
if __name__ == '__main__':
    dtr = ToRosbag('2013-01-10', 'test2.bag')
    dtr.process()
    #dtr.compress_bag()
